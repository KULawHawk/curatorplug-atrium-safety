...
- **2026-05-08 v0.4** — IMPLEMENTED. The audit-log integration that v0.3 §9 named as "out of scope, requires a separate `curator_audit_event` hookspec design+ratification cycle" is now SHIPPED. Scope of changes since v0.3:
  ...
  * **Atrium Constitution Principle 4 (No Silent Failures) constitutionally satisfied** for this plugin's enforcement decisions. Every decide → enforce decision the plugin makes is now a queryable audit row. `curator audit --actor curatorplug.atrium_safety` returns real data showing what the plugin did and why.
