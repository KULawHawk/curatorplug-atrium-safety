...
**Status:** v0.4 — IMPLEMENTED 2026-05-08. The original 3-session plan and the two follow-on plans (PLUGIN_INIT + CURATOR_AUDIT_EVENT) are all fully shipped end-to-end. Headline capability now includes structured audit emission for every enforcement decision (`compliance.approved` / `compliance.refused` / `compliance.warned` actions queryable via `curator audit-log query --actor curatorplug.atrium_safety`). DM-3's audit-log integration — ratified in v0.2 but deferred for implementation pending Curator-side plumbing — is now LIVE as of plugin v0.3.0 (commit 8910748, tag v0.3.0). All capabilities described in this design plus all DM ratifications are honored at runtime.
**Date:** 2026-05-08
...
- **2026-05-08 v0.2** — RATIFIED. Jake ratified all 4 DM recommendations (DM-1 through DM-4) as written without modification. Doc status flipped from "design proposal" to "approved spec"; P1 implementation cleared to begin. No structural changes to the design — only ratification-status flips on each DM and the v0.2 revision-log entry.
- **2026-05-08 v0.3** — IMPLEMENTED. The original 3-session plan and the follow-on PLUGIN_INIT plan are both fully shipped:
  * **P1 (Curator-side hookspec)** — shipped as Curator v1.1.1 (commit 75dc010, tag v1.1.1). Adds `curator_source_write_post(source_id, file_id, src_xxhash, written_bytes_len)` to the hookspec surface and fires it from `MigrationService._cross_source_transfer` after each successful write.
  * **P2 (plugin scaffolding + soft enforcement)** — shipped as plugin v0.1.0 (commits 728b0e0 P2 scaffolding + 1dff4ce P3 release, tag v0.1.0). 53 tests passing (45 unit + 8 integration). Strict mode refuses writes where `src_xxhash is None` (caller skipped verify); lax mode is a no-op observer.
  * **PLUGIN_INIT plan (re-read verification)** — the deferred capability noted in v0.1 §5 ("the atrium_safety plugin re-reads via `curator_source_read_bytes`...") needed Curator-side plumbing that was identified during P2 implementation: plugins lack pm-from-hookimpl access, blocking the re-read mechanism. A separate design (`Curator/docs/PLUGIN_INIT_HOOKSPEC_DESIGN.md` v0.2 RATIFIED) closed this gap with the `curator_plugin_init(pm)` hookspec. **P1 of that plan** shipped as Curator v1.1.2 (commit cd357fd, tag v1.1.2) — the hookspec + wiring + 6 unit tests (348/348 in slice). **P2 of that plan** shipped as plugin v0.2.0 (commit e28473c, tag v0.2.0) — the plugin's `curator_plugin_init` hookimpl + `_verify_via_re_read` two-phase enforcement + 18 new tests (71/71 total in plugin suite).
  * **§5 hook surface table is now LIVE** — every entry in the table is implemented, exercised by tests, and pushed to GitHub. Specifically: the row "the atrium_safety plugin re-reads via `curator_source_read_bytes`, recomputes xxh3, compares. On mismatch: raises `ComplianceError`" describes runtime behavior in plugin v0.2.0 strict mode, proven end-to-end by `tests/integration/test_curator_runtime.py::TestReReadVerificationAutoDiscovered::test_re_read_catches_post_write_corruption_in_strict`.
  * **§7 deferrals status:** Multi-algorithm hash verify (DM-2 (b)) remains deferred per its original gating (no shared-tenancy threat model has emerged). All other v0.1.0 deferrals are now resolved.
  * **What's next (out of scope for this design):** DM-3's audit-log integration is a known gap — the plugin currently logs at debug/warn level via `loguru` rather than writing structured audit entries via Curator's AuditRepository. That requires a separate `curator_audit_event` hookspec (or similar) that is its own design + ratification cycle. Tracked for a future release.
- **2026-05-08 v0.4** — IMPLEMENTED. The audit-log integration that v0.3 §9 named as "out of scope, requires a separate `curator_audit_event` hookspec design+ratification cycle" is now SHIPPED. Scope of changes since v0.3:
  * **CURATOR_AUDIT_EVENT plan executed.** Designed in `Curator/docs/CURATOR_AUDIT_EVENT_HOOKSPEC_DESIGN.md` v0.1 DRAFT → v0.2 RATIFIED → v0.3 IMPLEMENTED, all on 2026-05-08. **P1 of that plan** shipped as Curator v1.1.3 (commit 15c1f61, tag v1.1.3) — `curator_audit_event(actor, action, entity_type, entity_id, details)` hookspec + `AuditWriterPlugin` core plugin + 9 unit tests (357/357 in slice). **P2 of that plan** shipped as this plugin's v0.3.0 (commit 8910748, tag v0.3.0) — `_fire_audit_event` helper + audit emission at all 5 enforcement decision branches + 4 integration tests (75/75 total in plugin suite).
  * **DM-3 ratification now honored at runtime.** v0.2 DM-3 ratified `actor='curatorplug.atrium_safety'` for audit emissions, but the implementation path didn't exist. v0.3.0 now uses that exact actor string for every audit event. Three new actions in the audit log:
    - `compliance.approved` — every successful enforcement decision
    - `compliance.refused` — every refusal (decide-phase or re-read-phase)
    - `compliance.warned` — lax-mode re-read mismatch (advisory)
  * **Atrium Constitution Principle 4 (No Silent Failures) constitutionally satisfied** for this plugin's enforcement decisions. Every decide → enforce decision the plugin makes is now a queryable audit row. `curator audit-log query --actor curatorplug.atrium_safety` returns real data showing what the plugin did and why.
  * **§7 deferrals status (updated):** Multi-algorithm hash verify (DM-2 (b)) still deferred (unchanged). The v0.3 §9 "What's next" item about audit-log integration is now RESOLVED. New §9 "What's next" item below.
  * **§9 (NEW) what's next:** The plugin is now feature-complete relative to its v0.1 design + DM-3 ratification. Future enhancements would be additional designs in their own right — e.g., a `curatorplug-atrium-reversibility` plugin enforcing Constitution Principle 3 (Reversibility), or migration of MigrationService's own audit writes to fire through `curator_audit_event` (deferred per CURATOR_AUDIT_EVENT design DM-3 — revisit only if a real audit-aggregator-plugin use case emerges).
